...
struct WebsiteController: RouteCollection {
  ...
  func boot(router: Router) throws {
    ...
    protectedRoutes.post(
      CreateAcronymData.self, at: "acronyms", "create",
  ...
  func forgottenPasswordPostHandler(_ req: Request) throws -> Future<View> {
    ...
    return User.query(on: req)
      ...
      .flatMap(to: View.self) { user in
        guard let user = user else {
          return try req.view().render("forgottenPasswordConfirmed",
        ...
        return resetToken.save(on: req).flatMap(to: View.self) { _ in
          ...
          let fromEmail = EmailAddress(email: "hsinyichen@hsinyitech.com", name: "eric")
          ...
          return try sendGridClient.send([email], on: req.eventLoop)
            .flatMap(to: View.self) { _ in
              return try req.view()
                .render("forgottenPasswordConfirmed", ["title": "Password Reset Email Sent"])
          }
        }
    }
  }
